...
/* empty css                */
import { p as productsData } from "./productson.min.js";
import { b as renderCard } from "./renderCard.min.js";
let formValidate = {
  getErrors(form) {
    let error = 0;
  ...
  getSelectTitleValue(selectItem, originalSelect) {
    ...
    if (originalSelect.multiple && originalSelect.hasAttribute("data-fls-select-tags")) {
      selectTitleValue = this.getSelectedOptionsData(originalSelect).elements.map(
        (option) => `<span role="button" data-fls-select-id="${selectItem.dataset.flsSelectId}" data-fls-select-value="${option.value}" class="--select-tag">${this.getSelectElementContent(option)}</span>`
      ).join("");
...
const bodies = document.querySelectorAll(".tabs__body-filter");
tabs.forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    bodies.forEach((b) => {
      ...
      if (b.dataset.tabBody === id) b.classList.add("active");
    });
  });
});
var PipsMode;
...
function scope(target, options, originalOptions) {
  ...
  function moveHandles(upward, proposal, locations, handleNumbers, connect) {
    ...
    var state2 = false;
    handleNumbers.forEach(function(handleNumber, o) {
      state2 = setHandle(handleNumber, locations[handleNumber] + proposal, b[o], f[o], false, smoothSteps) || state2;
    });
    if (state2) {
...
document.addEventListener("DOMContentLoaded", () => {
  const catalogMobileFilters = document.querySelector(".aside-filter");
  ...
  const openFilter = () => {
    catalogMobileFilters.classList.add("aside-filter--mobile-visible");
    catalogOverlay == null ? void 0 : catalogOverlay.classList.add("overlay--visible");
    disableScroll();
  };
  const closeFilter = () => {
    catalogMobileFilters.classList.remove("aside-filter--mobile-visible");
...
const listContainer = document.querySelector("[data-products-category]");
const paginationWrapper = document.querySelector("[data-fls-pagination]");
const badgesContainer = document.querySelector(".catalog__filter-items");
const gridBtn = document.querySelector(".catalog__filter-btngrid");
const lineBtn = document.querySelector(".catalog__filter-btnline");
const clearBtns = document.querySelectorAll(".clear-filters");
const CATEGORY = listContainer == null ? void 0 : listContainer.dataset.productsCategory;
const allItems = productsData.filter((item) => item.category === CATEGORY);
const defaultState = () => ({
  filters: {
...
const parseNumber = (val) => {
  ...
  return parseFloat(val == null ? void 0 : val.toString().replace(/[^\d.-]/g, "")) || 0;
};
function saveState() {
...
function loadState() {
  try {
    ...
    return {
      ...def,
      ...parsed,
      filters: { ...def.filters, ...parsed.filters }
    };
  } catch (e) {
    console.warn("Failed to load catalog state:", e);
    return null;
  }
}
function updateURL() {
  ...
  if (f.price.min > 0 || f.price.max < MAX_PRICE) {
    params.set("price", `${f.price.min}-${f.price.max}`);
  }
...
function loadFromURL() {
  ...
  const f = state.filters;
  if (params.get("brand")) {
    f.brand = params.get("brand").split(",").map(normalize);
  }
  if (params.get("price")) {
    const [min, max] = params.get("price").split("-").map(Number);
    f.price = { min: min || 0, max: max || MAX_PRICE };
  }
}
function applyFilters() {
  ...
  filteredItems = allItems.filter((item) => {
    ...
    if (price < f.price.min || price > f.price.max) return false;
    if (f.availability.length) {
      if (!f.availability.includes(normalize(item.availability))) return false;
    }
    if (f.products.length) {
      const matchesAll = f.products.includes("all");
      const matchesNew = f.products.includes("new arrivals") && item.isNew;
      const matchesSale = f.products.includes("sale") && item.sale;
      if (!matchesAll && !matchesNew && !matchesSale) return false;
    }
    const skipKeys = ["price", "availability", "products"];
    const filterKeys = Object.keys(f).filter((key) => !skipKeys.includes(key));
    for (const key of filterKeys) {
      if (!f[key] || f[key].length === 0) continue;
      const rawValue = item[key] ?? item[key.toLowerCase()];
      if (rawValue === null || rawValue === void 0) continue;
      const itemValues = String(rawValue).split(",").map(normalize);
      const hasMatch = f[key].some(
        (filterVal) => itemValues.includes(normalize(filterVal))
      );
...
function render() {
  ...
  const itemsToShow = filteredItems.slice(start, start + ITEMS_PER_PAGE);
  if (itemsToShow.length === 0) {
    listContainer.innerHTML = `<p class="no-results">No results found for the selected filters.</p>`;
    renderPagination();
    return;
  }
  listContainer.innerHTML = itemsToShow.map(renderCard).join("");
  renderPagination();
}
function renderPagination() {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    btn.textContent = i;
    btn.className = `pagination__btn${i === state.page ? " pagination__btn--active" : ""}`;
...
function updateBadges() {
  ...
  if (f.price.min > 0 || f.price.max < MAX_PRICE) {
    badgesContainer.appendChild(
      createBadge(`$${f.price.min} – $${f.price.max}`, () => {
        var _a, _b;
        f.price = { min: 0, max: MAX_PRICE };
        (_b = (_a = document.getElementById("pricerange")) == null ? void 0 : _a.noUiSlider) == null ? void 0 : _b.set([0, MAX_PRICE]);
        applyFilters();
      })
    );
  }
  const filterKeys = Object.keys(f).filter((key) => key !== "price");
  for (const key of filterKeys) {
      ...
      badgesContainer.appendChild(
        createBadge(`${val}`, () => {
          ...
          document.querySelectorAll(`input[data-filter="${key}"]`).forEach((el) => {
            if (normalize(el.value) === normalize(val)) el.checked = false;
          });
          applyFilters();
        })
      );
...
function createBadge(text, onRemove) {
  const btn = document.createElement("button");
  btn.className = "catalog__filter-badge";
...
document.addEventListener("change", (e) => {
  ...
  if (!input.classList.contains("checkbox__input") && !input.matches(".tabs__body-filter input"))
    return;
  const key = input.dataset.filter ?? ((_a = input.name) == null ? void 0 : _a.replace("[]", ""));
  if (!key || !Object.prototype.hasOwnProperty.call(state.filters, key)) return;
...
document.addEventListener("priceChange", (e) => {
  state.filters.price = e.detail;
  applyFilters();
});
document.addEventListener("selectCallback", (e) => {
  const select = e.detail.select;
  if (!select.matches("[data-catalog-sort]")) return;
...
clearBtns.forEach((btn) => {
  btn.addEventListener("click", () => {
    var _a, _b;
    state = defaultState();
    document.querySelectorAll(".checkbox__input, .tabs__body-filter input").forEach((el) => el.checked = false);
    (_b = (_a = document.getElementById("pricerange")) == null ? void 0 : _a.noUiSlider) == null ? void 0 : _b.set([0, MAX_PRICE]);
    applyFilters();
  });
});
gridBtn == null ? void 0 : gridBtn.addEventListener(
  "click",
  () => listContainer.classList.remove("tabs-list__item--line")
);
lineBtn == null ? void 0 : lineBtn.addEventListener(
  "click",
  () => listContainer.classList.add("tabs-list__item--line")
);
function init() {
  loadFromURL();
