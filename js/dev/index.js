...
import "./productson.min.js";
import "./renderCard.min.js";
/*!
...
function jQueryFactory(window2, noGlobal) {
  ...
  function isObviousHtml(input2) {
    return input2[0] === "<" && input2[input2.length - 1] === ">" && input2.length >= 3;
  }
...
requireSlick();
function initSlick() {
  const $slider = jQuery("[data-fls-slick]");
  if ($slider.length) {
    if (!jQuery.type) {
      jQuery.type = (obj) => Object.prototype.toString.call(obj).slice(8, -1).toLowerCase();
    }
    ...
    console.log("Slick slider started!");
  }
}
...
const tabsNav = document.querySelector(".search [data-fls-tabs-titles]");
const input = document.getElementById("search-input");
const error = document.querySelector(".search__error");
const form = document.querySelector(".search__form");
let currentType = "number";
tabsNav.addEventListener("click", (e) => {
  const btn = e.target.closest(".search__tabs-item");
  if (!btn) return;
  tabsNav.querySelectorAll(".search__tabs-item").forEach((b) => {
...
form.addEventListener("submit", (e) => {
  ...
  hideError();
  console.log(`Searching for "${query}" as ${currentType}`);
});
